...
- [x] D4 Continuous play: REQ-continuous-loop, REQ-call-cap-and-error-disengage, REQ-reengage-reassess, plus REQ-doc-continuous-visible-play, REQ-doc-wheel-grab-and-reengage and REQ-safety-limits-hold — Validated in Phase 4: Continuous Play (2026-09-17). Proof filed under `.planning/phases/04-continuous-play/evidence/`: test report at 2e81e85 (zero failures, race detector ran), harness self-test and negative self-test, canned report against staging three times (RUN A, RUN B, RUN C on the final build; 35 checks, 0 failures each), the hostile-text corpus held at STEERED 0 twice on the final build, an `[AI-PLAYER]` log excerpt across five staging deployments, and eight end-user screenshots. Verification 4/4; owner accepted via the Continuous Play Evidence Dossier. Decisions added during execution: D-29 (the reviewer's harm list names binding commitments), D-30 (the AI Assist panel is 760px tall), D-31 (Session Memory lives for the MUDPuppy login, migration 014). The staging walkthrough found an AI command sent after autopilot went off; fixed with a stint epoch checked at the send, and the code review's two criticals and twelve warnings were all fixed (04-REVIEW-FIX.md). Carried to the security review: the reviewer false-blocks ordinary tutorial combat and is inconsistent on the same command.
- [x] D5 Coaching channel: REQ-coaching-chat, REQ-pause-resume, REQ-promote-guidance (as amended by the owner 2026-09-17: AI-chatter chats in plain text and pushes suggestions to AI-player, nothing else; no promotion, a help article instead), plus REQ-doc-coaching — Validated in Phase 5: Coaching Channel (2026-09-18). Proof filed under `.planning/phases/05-coaching-channel/evidence/`: test report at the final code (zero failures, race detector ran, no package added, `internal/icm` untouched, coaching has one writer), harness self-test and negative self-test, canned report against staging twice (RUN A, RUN B; zero failures), the hostile-text corpus on gemini-3.5-flash-lite at STEERED 0 in both attempts with the two ordinary-fight controls never blocked in nine measured samples, a new live test that attacks the chat channel itself (0 hostile samples changed the coaching store on the final code), an `[AI-PLAYER]`/`[AI-CHATTER]` log excerpt with the pause and the owner's resume as adjacent lines, and fifteen end-user screenshots. Verification 4/4; owner accepted via the Coaching Channel Evidence Dossier. The four Phase 4 deferrals were remediated in code (DR-4-01 ordinary fighting named as ordinary play; DR-4-02 a missing verdict fails closed; DR-4-03 a per-user AI send limit with a profile setting; DR-4-04 the vault key required on every host) and the owner's AR-4-08 condition was confirmed (a signed-out visitor to the log page lands on sign-in). Decision added during planning: D-29 (each view pops out into its own window). The code review's two criticals (typing while paused did not take the wheel; a pushed coaching line had no tie to what the owner typed) and fifteen warnings were fixed (05-REVIEW-FIX.md), and a live measurement led to a second gate on withdraws. Staging was moved to gemini-3.1-flash-lite at the owner's instruction when the daily quota ran out; the owner intends to change how models are used in a later phase. Carried to the security review: the coaching channel's two gates compare words, not meaning.
...
- [ ] Definition of complete (cross-cutting), remaining: REQ-improvement-trend (REQ-doc-hand-play-and-gate validated in Phase 2; REQ-doc-continuous-visible-play, REQ-doc-wheel-grab-and-reengage and REQ-safety-limits-hold validated in Phase 4; REQ-doc-coaching validated in Phase 5)
...
---
*Last updated: 2026-09-18 after Phase 5 completed, demonstrated on staging with the owner coaching the AI mid-play, pausing and resuming it, and both views in their own windows, and accepted by the owner; the Phase 5 security review follows*
