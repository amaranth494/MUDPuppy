...
**Deploy method and the owner's session (required by the plan's acceptance criteria; added after the security audit noted its absence, T-4-13).** Every staging deploy this phase was `railway up --detach -e staging -s MudPuppy` from the local checkout, never the dashboard; `public/` was rebuilt and committed before each deploy that changed the frontend; production was never deployed to and no Railway variable was changed. The owner was never signed out at any point: both walkthrough checkpoints were driven in his already signed-in browser, the only sign-in during the phase was his own at 13:44 UTC on 2026-09-17, and his session cookie was supplied by him in chat, used only as an environment variable for the three live harness runs, and appears in no file. The harness restores the goal itself (the `PASS C1` restore lines of RUN A, B and C); it was confirmed on screen rather than retyped.
