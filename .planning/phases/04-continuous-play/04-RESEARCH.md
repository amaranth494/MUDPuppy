...
1. **(RESOLVED for planning: tolerated by D-19's WAITING pause and resume; root cause out of scope) What is the actual root cause of Alter Aeon dropping the socket ~30 seconds after every reconnect?**
...
2. **(RESOLVED for planning: measured during the staging walkthrough; ceilings set conservatively per A8) Actual token/latency cost of the extended answer schema on `gemini-3.5-flash-lite`**
