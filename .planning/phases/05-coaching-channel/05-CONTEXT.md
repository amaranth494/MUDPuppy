...
- **D-29 (added by the owner 2026-09-17 during the UI design contract):** Owner's words: "would it be possible to pop the window out into a disconnected window? Same for the AI-Player?" **Each of the two views can be popped out into its own separate browser window**: one pop-out button on the AI-player view, one on the AI-chatter conversation. Whichever view is not popped out stays docked in the panel (D-03's docked split is the default and is unchanged). A popped-out window is fed by the play screen's own live data (no second sign-in, no second game connection), so it lives only while the play screen tab is open; closing the pop-out returns that view to the panel. Everything that works docked works the same popped out (message box, Pause/Resume, Coaching in effect, markers, notices). The docked conversation area stays a fixed height, not resizable. This amends D-03's "no new pane" only to this extent.
- **Owner confirmations from the UI design contract (2026-09-17):** the "Coaching received" marker does not repeat the coaching text (confirms D-05); the Logs page shows the conversation as its own section below the transcript, not woven in; the Pause/Resume button is greyed out, not hidden, when autopilot is fully off.
