...
1. **Does the owner want a bounded WAITING lifetime (auto-land-on-OFF after N minutes disconnected), given the deferred security concern that WAITING→ON resume can happen while the owner is away?**
...
2. **Exact shape of the `#AUTO` REST request/response and whether `connection_id` travels in the body or is resolved some other way.**
