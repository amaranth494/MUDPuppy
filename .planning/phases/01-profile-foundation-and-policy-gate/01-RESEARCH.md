...
**Warning signs:** Migration `010` runs clean locally but a staging database inspection (the phase's own diagnostic) shows the columns missing.
...
1. **Does the deployed (Railway staging) process definitely run `migrate.Up()` from the same `migrations/` directory checked into this branch, or from a separately-packaged `migrate.zip` that could lag behind?**
   ...
   - Recommendation: the plan's verification step should explicitly include running the migration and doing the "database inspection" diagnostic against Railway staging itself (not just a local Postgres), which will settle this empirically regardless of which mechanism is actually active.

2. **Exact validation limits for `conduct_rules`/`approach_guidance` text length and `model_name` length.**
...
| REQ-profile-ai-fields | New columns present on a migrated `profiles` row with correct types/defaults | diagnostic (DB inspection) | manual/Railway console query: `SELECT conduct_rules, approach_guidance, ai_settings, policy_version_accepted, policy_accepted_at FROM profiles LIMIT 1;` | n/a (not a `go test`) |
...
- **Phase gate:** Full suite green (or at minimum, no new failures beyond the pre-existing `continue-on-error: true` baseline) before `/gsd:verify-work`, plus the player-observable browser walkthrough and the staging database inspection described in the roadmap's Phase Validation line.
