...
| 01-05-03 | 05 | 4 | REQ-policy-gate | T-1-05 | Fresh profile shows policy first with no editor; one Accept reveals the acceptance line and editor; the gate refusal is legible | player-observable (screenshots) | n/a — image evidence | `evidence/05-policy-first.png`, `evidence/06-accepted-line.png`, `evidence/12-gate-refusal.png` | ⬜ pending |
...
| Engage-gate refusal body is legible to the owner | REQ-policy-gate | Refusal is an API response, shown via devtools or curl | Call `GET /api/v1/profiles/<connection_id>/engage-gate` before accepting; screenshot the devtools Network response pane, the curl output, or the in-app refusal banner | `evidence/12-gate-refusal.png` |
