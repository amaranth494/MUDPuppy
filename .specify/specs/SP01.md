...
**Special QA Tests (Critical):**

1. **Session Hard Cap (24h):**
...
2. **OTP Reuse (Race Condition):**
   - Fire two concurrent login attempts with same OTP
   - Ensure only one succeeds, other fails cleanly (not 500)
   - Validates atomic behavior under concurrency

3. **Middleware Coverage:**
   - Test /api/v1/logout without session → should return 401
   - Test /api/v1/login with malformed payload → should return 400
   - Test any stubbed /api/v1/* routes → all return 401 without session
...
- [x] Feature works in Railway
- [x] CI passes
- [x] QA phase passes (browser-only validation)
- [x] /health unchanged
- [x] Production URL stable
- [x] No secrets exposed
- [x] Migration verified safe
- [x] Logout invalidates session correctly
