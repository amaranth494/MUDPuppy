...
- **Deny-listed dangerous ports (always blocked):**
...
- **Allowed ports (with constraints):** 23 (telnet), 2525, and any other non-deny-listed port
- **Standard guardrails:** Private IP blocking, one connection per user, rate limiting, message size cap (64KB), session duration cap
- **Protocol Plausibility Check (optional):** Disconnect if first N seconds/X bytes show TLS handshake, HTTP headers, or obvious binary protocol
